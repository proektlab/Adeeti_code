...
times = whiteScreenFlash(fParam.flashDelay, fParam.numFlashes, fParam.flashDuration, fParam.waitBlack, w, params, lj);

printParams(fParam, times, zeros(size(times)), startTime, [stimDir, 'Whole_Screen_Flash', datestr(now) ]);

%% Change locations white box

% setting up locations 
numTrialsPerCenter = 1;
...
printParams(swParam, times, permLocations, startTime, [stimDir, 'Corners_Flash', datestr(now) ]);
...
printParams(bDParam, times, zeros(size(times)), startTime, [stimDir, 'Big_Static', datestr(now) ]);
...
printParams(bDParam, times, zeros(size(times)), startTime, [stimDir, 'Big_Drift', datestr(now) ]);
...
printParams(bDParam, times, permLocations, startTime, [stimDir, 'Corners_Static', datestr(now) ]);
...
printParams(bDParam, times, permLocations', startTime, [stimDir, 'Corners_Drift', datestr(now) ]);
...
printParams(cRParam, times, zeros(size(times)), startTime, [stimDir, 'Big_Contrast', datestr(now) ]);
%% End of Experiment
sca;
Priority(0);
lj.close;
