%function COPYDriftAndTone2([angle = 30], [cyclespersecond = 1], [f = 0.01], [drawmask = 1], [maskStd = 90], [gratingsize = 400], [toneFs = 44100], [toneFreq = 4000], [sigFreq = 0.5], [duration = 10])
...
%   duration = duration of movie and sound.


function SuperiorSoundandVisual(params, w, angle, cyclespersecond, cpd, gratingsize, toneFreq, sigFreq, duration, TTLParams)
%% timing stuff
    % make sure the delay for the set up is exactly params.delay
    vbl = GetSecs() + params.delay;
%% defaults
    if nargin < 1
        angle = [];
    end
...
%% actual code
    ...
    signal = removeClick(signal, 300, params.toneFs);
...
% This is the part of the code that's already in numberoftrials
...
% end of area in numberoftrials
    ...
    % Create one single static grating image: 
        ...
        mask(:, :, 2)= params.white * (1 - exp(-((x/params.maskStd).^2)-((y/params.maskStd).^2)));
        ...
        dstRect=CenterRect(dstRect, params.windowRect);
    ...
    % Meat of audio coding
        ...
        if TTLParams ~= 0
            lj.toggleFIO(TTLParams); %send pulse
        end

        % Start audio playback for 'repetitions' repetitions of the sound data,
        % start it immediately (0) and wait for the playback to start, return onset
        % timestamp.
        PsychPortAudio('Start', pahandle, 1, vbl + (waitframes - 0.5) * ifi, 1);
    ...
    % Animationloop:
        ...
        %textures.
        % sca;
        %To close the textures on window in case sound trial is after
