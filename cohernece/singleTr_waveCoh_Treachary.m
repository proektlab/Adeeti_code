...
allDir = [dir('GL1*'); dir('*GL7'); dir('*GL8'); dir('*GL9');];
...
for d = 1:length(allDir)
    ...
    for a = 1:length(allData)
        ...
        for ch1 = 1:length(goodChan) %size(meanSubData, 1)
            ...
            eval([['cohMag_', num2str(ch1)] '= CohMag;'])
            tempMag = ['cohMag_', num2str(ch1)];
            ...
            if ch1 ==1 || ~exist([dirWCoh, allData(a).name(1:end-4), 'wave.mat']) ==1
                ...
                save([dirWCoh, allData(a).name], tempMag, 'actFreq', 'info','uniqueSeries', 'indexSeries')
