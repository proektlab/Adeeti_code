...
dirPic = '/synology/adeeti/ecog/images/gratingTesting/GT4/';
dirOut = dirIn;

driveLocation = '/home/adeeti/Dropbox/';  %'/data/adeeti/Dropbox/'; %'/home/adeeti/googleDrive/'; %dropbox location for excel file and saving

excelFileName = 'ProektLab_code/Adeeti_code/preprocessing/gratingsTesting.xlsx';
excelSheet = 4;
...
if isfield(dataMatrixFlashes, 'numberStim')
    numStim = max([dataMatrixFlashes.numberStim]);
    if numel(numStim) > 1
    ...
    if numel(numStim) == 0
        ...
        for i = 1:length(dataMatrixFlashes)
            for j = 1:size(dataMatrixFlashes(i).stimIndex)
                if numStim == 2
                    matStimIndex(n,:) = dataMatrixFlashes(i).stimIndex(j,:);
                elseif numStim == 3
                    matStimIndex(n,:,:) = dataMatrixFlashes(i).stimIndex(j,:,:);
                elseif numStim ==4
                    matStimIndex(n,:,:,:) = dataMatrixFlashes(i).stimIndex(j,:,:,:);
...
for experiment = START_AT:length(allData)
    ...
    % making images
    for stimIndex= 1:size(uniqueSeries, 1)
        strStimInd = uniqueSeries(stimIndex,:);
        [stimIndexSeriesString] = stimIndex2string4saving(strStimInd, finalSampR);
        
        %Single trial images
        [indices] = getStimIndices(uniqueSeries(stimIndex,:), indexSeries, uniqueSeries);
        useMeanSubData = meanSubData(:,indices,:); 
        ...
        [currentFig] = plotAverages(squeeze(aveTrace(stimIndex,:,:)), finalTime, info, [], [], [],[], before, after, flashOn, finalSampR);
        ...
        saveas(currentFig, [dirPic, info.expName(1:end-4), '_', stimIndexSeriesString, '_ave.png'])
