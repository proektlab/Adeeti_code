function [Out] = mtImageFFT_AA(image,tapers, imDim, freqsOfInterest, padding)
...
[out,~,~] = swTFspecAnalog(image,s2,tapers,freqsOfInterest,s2,1,[],padding,[],[],[]); % perform multitaper along second dimension
%[out,taper,concentration] = swTFspecAnalog(data,sampleFreq,Ktapers,freqsOfInterest,window,winstep,NW,pad,taper,concentration,doAdapt,includePhase)
...
[out,taper,concentration] = swTFspecAnalog(CompSpect.',s1,tapers,freqsOfInterest,s1,1,[],padding,[],[],[]);
