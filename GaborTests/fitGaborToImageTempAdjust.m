function [fitParameters, score, correlation, gridX, gridY] = fitGaborToImageTempAdjust(movieToFit, timept, timeptsFit, debug)
%[fitParameters, score, correlation, gridX, gridY] = fitGaborToImage(imageToFit, timepointsFit, debug)

if ~exist('movieToFit') || isempty(movieToFit)
    movieToFit = rand(100, 100);
end

if ~exist('timept') || isempty(timept)
    timept = 1;
end

if ~exist('timeptsFit') || isempty(timeptsFit)
    timeptsFit = 1;
end
...
if timeptsFit ==1
    imageToFit = squeeze(movieToFit(timept,:,:));
else
    movieSeg = movieToFit(timept:timept+timeptsFit-1,:,:);
    imageToFit = squeeze(nanmean(movieSeg,1));
end
