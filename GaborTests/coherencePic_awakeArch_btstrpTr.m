    ...
    for g = 1:length(allMiceAwa)
        ...
        for d = startD:length(allDir)
            ...
            for a = 1:length(MFE)
                ...
                clearvars interp1Coh35
                
               % [rawFiltDataTimes, interpFiltDataTimes, info] = makeStillEcogGrids(experiment, steps, fr, 3, stimIndex, 0, 1);
                [~, interp1Coh35, info] = makeStillEcogGrids(experiment, steps, fr, 1, stimIndex, 0, 1);
                save([dirOut, 'gaborCoh', experiment(1:end-8), '.mat'], 'interp1Coh35', 'info')
