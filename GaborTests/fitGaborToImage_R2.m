function [fitParameters, score, correlation, gridX, gridY, parameters] = fitGaborToImage_R2(imageToFit, debug, expParams, lambda)
...
%         errorImage = nansum((abs(tempImage(:)) .* imagePower(:)).^2);
        ...
        %R2ed = SSReg/SST;
        R2ed = 1- (SSRes/SST);
        errorImage = (1-R2ed); %nansum(abs((1-R2ed)*imagePower(:).^2));
